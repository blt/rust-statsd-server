...
impl Wavefront {
    ...
    pub fn format_stats(&self, buckets: &Buckets, curtime: Option<i64>) -> String {
        ...
        for (key, value) in buckets.timer_data() {
            write!(stats,
                   "{} {} {} source={}\n",
                   key,
                   value,
...
mod test {
    ...
    fn test_format_buckets_timers() {
        ...
        assert!(result.contains("test.timer.max 12.101 10101 source=test-src"));
        assert!(result.contains("test.timer.min 1.101 10101 source=test-src"));
        assert!(result.contains("test.timer.count 3 10101 source=test-src"));
    }
}
